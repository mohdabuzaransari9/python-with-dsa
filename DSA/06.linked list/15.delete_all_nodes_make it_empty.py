...
class LinkedList:
    ...
    def remove(self,index): 
        #special edge case reminder in here
        if index<-1 or index>=self.length: # linked list out of range
            return None
        if index==0:     # if linked list has only one element or remove only first
            return self.pop_first()
        
        if index==self.length-1 or index==-1: # special edge case tail still point to popped node so to correct this we do this
            return self.pop()
